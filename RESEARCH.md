...
The normalized backend and native Android path described above reached v0.4.0 on 2026-07-29. It is distributed directly from `berthalla.io/widget`, outside the Play Store, with Compact 2×2 and Market 4×2 widgets, a locally stored holdings-value feature, and Theme Studio. Theme Studio provides distinct Home and Lock artwork for Mayor Purple, Woofhub Night, and Berthalla Nights plus automatically matched widget palettes. The Compact widget builds a real rolling sparkline from validated observations accumulated during refreshes, rather than presenting the illustrative history from the design as live data. The history is private, capped at 192 observations, and limited to 24 hours. The source repository is public and linked from the landing page alongside the signed release manifest and APK checksum. The iOS source includes corresponding wallpaper-sharing flows but remains unshipped because WidgetKit compilation, device validation, signing, and TestFlight distribution require macOS and an Apple Developer team.
