...
The normalized backend and native Android path described above reached v0.3.8 on 2026-07-28. It is distributed directly from `berthalla.io/widget`, outside the Play Store, with Compact 2×2 and Market 4×2 widgets plus a locally stored holdings-value feature. The v0.3.8 design pass makes tall 4×2 launcher allocations more useful with larger hero content, metric panels, and source/freshness context while retaining Jetpack Glance compatibility. It deliberately omits the illustrative design sparkline because the current quote API does not provide a historical series. The source repository is public and linked from the landing page alongside the signed release manifest and APK checksum. The iOS source is still unshipped because WidgetKit compilation, device validation, signing, and TestFlight distribution require macOS and an Apple Developer team.
