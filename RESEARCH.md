...
The normalized backend and native Android path described above reached v0.3.7 on 2026-07-28. It is distributed directly from `berthalla.io/widget`, outside the Play Store, with Compact 2×2 and Market 4×2 widgets plus a locally stored holdings-value feature. The v0.3.7 design pass adds clearer live/delayed state, stronger price and movement hierarchy, and a dedicated position panel while retaining Jetpack Glance compatibility. It deliberately omits the illustrative design sparkline because the current quote API does not provide a historical series. The source repository is public and linked from the landing page alongside the signed release manifest and APK checksum. The iOS source is still unshipped because WidgetKit compilation, device validation, signing, and TestFlight distribution require macOS and an Apple Developer team.
